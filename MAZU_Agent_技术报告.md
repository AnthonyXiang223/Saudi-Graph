...
1. **Janowicz, K., Hitzler, P., Li, W., et al. (2022).** *"Know, Know Where, KnowWhereGraph: A densely connected, cross-domain knowledge graph and geo-enrichment service stack for applications in environmental intelligence."* AI Magazine, 43(1), 30–39.
   - **链接**: https://doi.org/10.1002/aaai.12043
   - **相关性**: 本项目的核心知识底座参考架构。KWG 的 DMDO 灾害本体和 SOSA/SSN 观测模型直接指导了沙特极端天气知识图谱的设计。

2. **Shimizu, C., Stephen, S., et al. (2024).** *"The KnowWhereGraph Ontology."* Journal of Web Semantics, 84, 100842.
   - **链接**: https://doi.org/10.1016/j.websem.2024.100842
   - **相关性**: KWG 本体的权威文档，定义了 DMDO 的类层级、关系和 MOMo（Modular Ontology Modeling）建模方法论。本项目裁剪 DMDO 灾后属性、保留预警导向属性的决策依据于此。

3. **Tian, Y. & Li, W. (2022).** *"GeoAI for Knowledge Graph Construction: Identifying Causality Between Cascading Events to Support Environmental Resilience Research."* arXiv:2211.06011.
   - **链接**: https://arxiv.org/abs/2211.06011
   - **相关性**: 提供了级联事件因果链建模的三阶段方法（主题关联→时空共现→文本挖掘），本项目 OWL-Time 级联事件链借鉴了其事件因果推理思路。

### W3C 语义标准

4. **W3C (2017).** *"Semantic Sensor Network Ontology (SOSA/SSN)."* W3C Recommendation.
   - **链接**: https://www.w3.org/TR/vocab-ssn/
   - **相关性**: 本项目中所有 Observation、Sensor、FeatureOfInterest、Procedure 的建模基础。91 个气象指标作为 `sosa:ObservableProperty`，6 个数据源作为 `sosa:Sensor`/`ssn:System`。

5. **OGC (2012).** *"GeoSPARQL — A Geographic Query Language for RDF Data."* OGC 11-052r4.
   - **链接**: https://www.ogc.org/standard/geosparql/
   - **相关性**: 本项目空间查询的基础。`geof:distance` 用于半径搜索（如"利雅得 100km 内高温格点"），`geof:sfIntersects` 用于区域包含查询。

6. **W3C (2017).** *"OWL-Time Ontology."* W3C Recommendation.
   - **链接**: https://www.w3.org/TR/owl-time/
   - **相关性**: 本项目中 `time:Instant`、`time:Duration`、`time:before/after` 的时间建模依据，支持跨日期级联事件的时序推理。

7. **W3C (2013).** *"PROV-O: The PROV Ontology."* W3C Recommendation.
   - **链接**: https://www.w3.org/TR/prov-o/
   - **相关性**: 本项目中指标推导链（`prov:wasDerivedFrom`）、事件生成活动（`prov:wasGeneratedBy`）和数据源归属（`prov:wasAttributedTo`）的溯源建模基础。

### 极端天气与灾害知识图谱

8. **Zhang, W., Peng, L., Ge, X., et al. (2023).** *"Spatio-Temporal Knowledge Graph-Based Research on Agro-Meteorological Disaster Monitoring."* Remote Sensing, 15(18), 4403.
   - **链接**: https://doi.org/10.3390/rs15184403
   - **相关性**: 与本项目最为接近的时空知识图谱工作——针对河南冬小麦干热风灾害，融合 MODIS NDVI 遥感 + ESA 气象再分析数据，使用 OWL + GeoSPARQL + SWRLTO 构建农田斑块级监测系统。本项目的四类灾害检测规则借鉴了其阈值条件组合设计思路。

9. **Fang, Y., Zhang, D., & Wu, G. (2023).** *"Toward establishing a knowledge graph for drought disaster based on ontology design and named entity recognition."* Journal of Hydroinformatics, 25(4), 1457–1470.
   - **链接**: https://doi.org/10.2166/hydro.2023.046
   - **相关性**: 展示了"本体设计 + 数据驱动 NER 填充"的混合构建范式。本项目当前采用专家驱动方式定义 91 个算子，未来可借鉴其 BERT-BiLSTM-CRF 方法实现文献知识的自动化抽取。

10. **Wang, H.-M. & He, X. (2025).** *"DOMINO-SEE: Detection Of Multi-layer INterconnected Occurrences for Spatial Extreme Events."* GitHub: PREP-NexT/DOMINO-SEE.
    - **链接**: https://github.com/PREP-NexT/DOMINO-SEE
    - **相关性**: 空间极端事件网络检测框架，原生支持 xarray/NetCDF 数据和 Event Coincidence Analysis 网络分析。本项目的事件检测 pipeline 在架构上与其事件化→时空网络→统计检验的三阶段流程一致。

### 智能体 (Agent) 架构

11. **Yao, S., Zhao, J., Yu, D., et al. (2023).** *"ReAct: Synergizing Reasoning and Acting in Language Models."* ICLR 2023.
    - **链接**: https://arxiv.org/abs/2210.03629
    - **相关性**: 定义了 Reasoning + Acting 的 Agent 范式，本项目第 3.4 节的 ReAct 动态循环直接基于此框架设计。
...
12. **Wei, J., Wang, X., Schuurmans, D., et al. (2022).** *"Chain-of-Thought Prompting Elicits Reasoning in Large Language Models."* NeurIPS 2022.
    ...
    - **相关性**: 本项目第 4.2 节结构化思维链约束（Observation→Reasoning→Action）的理论基础。
...
13. **Hersbach, H., et al. (2020).** *"The ERA5 global reanalysis."* Quarterly Journal of the Royal Meteorological Society, 146(730), 1999–2049.
    - **链接**: https://doi.org/10.1002/qj.3803
    - **相关性**: 本项目底层气象数据（DS1/DS2/DS4）的来源。91 个指标中有 49 个直接源自 ERA5 日值和月值再分析产品。

14. **Huffman, G.J., et al. (2019).** *"GPM IMERG Final Precipitation L3 1 day 0.1 degree."* NASA GES DISC.
    - **链接**: https://doi.org/10.5067/GPM/IMERGDF/DAY/07
    - **相关性**: 本项目 DS10 卫星降水数据来源，为山洪短历时强降水检测（`ds10_max_1h`, `ds10_max_30min`）提供关键输入。
