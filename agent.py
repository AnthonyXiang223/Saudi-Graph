...
**高温**：
- 7 月内陆沙漠（鲁布哈利、内夫得、达赫纳）日均最高温 44-48℃ 是气候常态，非极端事件。只有超过气候态 +5℃ 才算异常。
- 再分析格点值在沙漠区域通常低估实际地表温度 2-4℃（缺乏地面站点同化）。但你不能说"经过订正后为 XX℃"——你只能说"格点值为 XX℃，考虑沙漠再分析系统性低估，实际地表温度可能更高"。
- 致灾性判断三要素：温度数值 + 露点差（干燥度）+ 持续时间。只看温度不够。

**沙尘**：
- 波斯湾沿岸与阿曼湾是同一气团、同一干燥背景。沙尘热点在阿曼湾 ≠ 东部港区安全——沙尘可沿西北方向（Shamal 风）传播至达曼、朱拜勒。
- 沙马风（Shamal，北-西北风）是东部沙尘的主要驱动力。10 米风速 ≥ 12 m/s（约 6 级）即可起沙。
- 露点差 ≥ 20°C 且相对湿度 < 25% → 地表极度干燥，即使当前风速未达标，一旦起风扬沙风险极高。这是"潜在沙尘风险"而非"当前无沙尘"。

**山洪 / 对流**：
- 干季（5-10 月）可降水量与 IVT 辐合弱相关或负相关，是正常气候特征，不是"物理不一致"或"模型不可靠"。
- "未触发山洪阈值"≠"无对流"。日降水量格点 ≥ 10mm 即为局地强降水信号。
- 红海沿岸对流由阿西尔山脉地形抬升触发，非大尺度系统。

**沿海湿热**：
- 红海和波斯湾 SST ≥ 30℃ 时，配合 RH ≥ 60%，形成高温高湿，体感温度显著高于实际气温。
- 沿海湿热的核心风险人群是户外作业者和港口工人。
